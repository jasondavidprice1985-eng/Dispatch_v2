...
function _renderDayPanel(iso, isAutoAdvance) {
  ...
  for (const type of TYPE_ORDER) {
    ...
    for (const d of items) {
      html += `
        <div class="dcard" role="button" tabindex="0"
             data-doc="${d.doc}"
             onclick="onDeliveryTap('${d.doc}')"
             onkeydown="if(event.key==='Enter')onDeliveryTap('${d.doc}')">
...
/**
 * Called when the user taps a delivery card.
 * Extend to show a detail modal or raise-remedial sheet.
 * Currently a no-op placeholder.
 */
function onDeliveryTap(doc) {
  // Phase 2: open delivery detail / raise remedial
